...
# (gitignored). Every runtime fetches it itself before grading, so this
# target is only for fetching it by hand; it is not a prerequisite of
# `make test`. When the corpus is missing the conformance tests FAIL,
# they never skip.
toml-test:
	bash scripts/fetch-toml-test.sh
...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/toml.go
	rm -f go/toml.go.bak
	git add go/toml.go
	git commit -m "go: v$(V)"
	git tag go/v$(V)
...
# doctests of the crate, so both lines are needed.
test-rs:
...
# A release that bumps the TS and Go sites and forgets these fails that
# test.
#
...
# not accept a path dependency, so tabnas-toml is not published. Only the
