# libtabnastoml — the toml parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The toml format parser as a C shared library, so languages with no
tabnas port can parse and validate toml input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnastoml","format":"toml","template":"v1"}` |
...
// link against libtabnastoml; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
